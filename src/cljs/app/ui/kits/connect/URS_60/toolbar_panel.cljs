    ...
    (when show-toolbar
      (<>
        ...
        ($ toolbar/switch-panel {:x 707 :y 12}
           ($ toolbar/switch {:x 0 :value switch-1 :on-change #(dispatch [:URS-60/change [[:switches 1] (not switch-1)]]) :legend 1 :inactive inactive})
           ($ toolbar/switch {:x 35 :value switch-2 :on-change #(dispatch [:URS-60/change [[:switches 2] (not switch-2)]]) :legend 2 :inactive inactive})
           ($ toolbar/switch {:x 70 :value switch-3 :on-change #(dispatch [:URS-60/change [[:switches 3] (not switch-3)]]) :legend 3 :inactive inactive})
           ($ toolbar/switch {:x 105 :value switch-4 :on-change #(dispatch [:URS-60/change [[:switches 4] (not switch-4)]]) :legend 4 :inactive inactive})
           ($ toolbar/switch {:x 140 :value switch-5 :on-change #(dispatch [:URS-60/change [[:switches 5] (not switch-5)]]) :legend 5 :inactive inactive})
           ($ toolbar/switch {:x 175 :value switch-6 :on-change #(dispatch [:URS-60/change [[:switches 6] (not switch-6)]]) :legend 6 :inactive inactive})
           ($ toolbar/switch {:x 210 :value switch-7 :on-change #(dispatch [:URS-60/change [[:switches 7] (not switch-7)]]) :legend 7 :inactive inactive})
           ($ toolbar/switch {:x 245 :value switch-8 :on-change #(dispatch [:URS-60/change [[:switches 8] (not switch-8)]]) :legend 8 :inactive inactive})
           ($ toolbar/switch {:x 280 :value switch-9 :on-change #(dispatch [:URS-60/change [[:switches 9] (not switch-9)]]) :legend 9 :inactive inactive})
           ($ toolbar/switch {:x 315 :value switch-10 :on-change #(dispatch [:URS-60/change [[:switches 10] (not switch-10)]]) :legend 10 :inactive inactive}))

        ($ toolbar/btn-wrapper {:x 1064 :y 54}
           ($ toolbar/btn-divider))

        ($ toolbar/btn-wrapper {:x 1114 :y 54 :on-click #(dispatch [:URS-60/change [[:full-screen?] (not full-screen?)]])}
