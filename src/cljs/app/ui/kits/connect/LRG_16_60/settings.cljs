(ns app.ui.kits.connect.LRG-16-60.settings
...
(defnc correction-dirty-factor []
  (let [value (use-sub [:kit/data-by-path [:cond :probes 0 "LRG 16-60" :correction-dirty :factor]])
    ...
    ($ f/field-row {:label "CELL CONSTANT"}
       ($ f/input-number {:value     value
                          :on-change #(dispatch [:LRG-16-60/change [[:correction-dirty :factor] %]])
                          :on-blur   (fn [] (f/validate-input-number value
                                                                     (:min limits)
                                                                     (:max limits)
                                                                     validate-message
                                                                     #(dispatch [:LRG-16-60/change [[:correction-dirty :factor] %]])))
                          :&         limits})
...
(defnc correction-dirty-temp-coeff []
  (let [value (use-sub [:kit/data-by-path [:cond :probes 0 "LRG 16-60" :correction-dirty :temperature-coeff]])
    ...
    ($ f/field-row {:label "TEMPERATURE COEFFICIENT"}
       ($ f/input-number {:value     value
                          :on-change #(dispatch [:LRG-16-60/change [[:correction-dirty :temperature-coeff] %]])
                          :on-blur   (fn [] (f/validate-input-number value
                                                                     (:min limits)
                                                                     (:max limits)
                                                                     validate-message
                                                                     #(dispatch [:LRG-16-60/change [[:correction-dirty :temperature-coeff] %]])))
                          :&         limits})
...
(defnc LRG-16-60-settings []
  (let [show-settings (use-sub [:kit/data-by-path [:cond :probes 0 "LRG 16-60" :settings-view]])]
    (when show-settings
      ($ modal/settings {:title      "Dirty Data Settings"
                         :on-default #(dispatch [:LRG-16-60/restore-defaults-settings])
                         :on-done    #(dispatch [:LRG-16-60/apply-settings])}
