(ns app.ui.kits.connect.NRR-2-60-3C.toolbar-panel
...
(defnc NRR-2-60-3C-toolbar-panel []
  (let [show-toolbar (use-sub [:NRR-2-60-3C/show-toolbar])
        ...
        switch-4 (use-sub [:kit/data-by-path [:level :controllers "NRR 2-60 3C" :switches 4]])]
    (when show-toolbar
      (<>
        ...
        ($ toolbar/switch-panel {:x 808 :y 12}
           ($ toolbar/switch {:x 0 :value switch-1 :on-change #(dispatch [:NRR-2-60-3C/change [[:switches 1] (not switch-1)]]) :legend 1 :inactive inactive})
           ($ toolbar/switch {:x 35 :value switch-2 :on-change #(dispatch [:NRR-2-60-3C/change [[:switches 2] (not switch-2)]]) :legend 2 :inactive inactive})
           ($ toolbar/switch {:x 70 :value switch-3 :on-change #(dispatch [:NRR-2-60-3C/change [[:switches 3] (not switch-3)]]) :legend 3 :inactive inactive})
           ($ toolbar/switch {:x 105 :value switch-4 :on-change #(dispatch [:NRR-2-60-3C/change [[:switches 4] (not switch-4)]]) :legend 4 :inactive inactive}))

        ($ toolbar/btn-wrapper {:x 954 :y 54}
           ($ toolbar/btn-divider))

        ($ toolbar/btn-wrapper {:x 1004 :y 54 :on-click #(dispatch [:NRR-2-60-3C/change [[:full-screen?] (not full-screen)]])}
           ($ toolbar/btn-full-screen))

        ($ toolbar/btn-wrapper {:x 1064 :y 54}
           ($ toolbar/btn-divider))

        ($ toolbar/btn-wrapper {:x 1114 :y 54 :inactive inactive :on-click #(dispatch [:NRR-2-60-3C/change [[:settings-view] true]])}
