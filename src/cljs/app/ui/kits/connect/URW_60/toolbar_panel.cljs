    ...
    (when show-toolbar
      (<>
        ...
        ($ toolbar/switch-panel {:x 917 :y 12}
           ($ toolbar/switch {:x 0 :value switch-1 :on-change #(dispatch [:change-URW-60 [[:switches 1] (not switch-1)]]) :legend 1 :inactive inactive})
           ($ toolbar/switch {:x 35 :value switch-2 :on-change #(dispatch [:change-URW-60 [[:switches 2] (not switch-2)]]) :legend 2 :inactive inactive})
           ($ toolbar/switch {:x 70 :value switch-3 :on-change #(dispatch [:change-URW-60 [[:switches 3] (not switch-3)]]) :legend 3 :inactive inactive})
           ($ toolbar/switch {:x 105 :value switch-4 :on-change #(dispatch [:change-URW-60 [[:switches 4] (not switch-4)]]) :legend 4 :inactive inactive}))
