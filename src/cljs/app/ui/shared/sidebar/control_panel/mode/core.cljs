  ...
  (let [mode (use-sub [:kit-data-by-path [:general-settings :operation-mode]])]
    (<>
      ...
      ($ mode-button {:x        -4
                      :label    "NORMAL"
                      :active   (= mode "normal")
                      :on-click #(dispatch [:change-kit-data-by-path [[:general-settings :operation-mode] "normal"]])})
      ...
      ($ mode-button {:x        132
                      :label    "STANDBY"
                      :active   (= mode "standby")
                      :on-click #(dispatch [:change-kit-data-by-path [[:general-settings :operation-mode] "standby"]])}))))
