  ...
  (let [show-toolbar-panel (use-sub [:sludge-valve/show-toolbar-panel])]
    (when show-toolbar-panel
      (<>
        ...
        ($ toolbar/btn-wrapper {:x 1114 :y 54 :on-click #(dispatch [:sludge-valve/change [[:settings-view] true]])}
           ($ toolbar/btn-settings))

        ($ toolbar/btn-wrapper {:x 1174 :y 54}
           ($ toolbar/btn-divider))

        ($ toolbar/btn-wrapper {:x 1224 :y 54 :on-click #(dispatch [:modal-info/change {:title "Bottom Blowdown Valve"
