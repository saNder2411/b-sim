  ...
  (let [value (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :mode :value]])
    ...
    ($ f/field-row {:label "CONTROL MODE"}
       ($ f/select {:data      [{:value "auto" :label "Auto"} {:value "manual" :label "Manual"}]
                    :value     value
                    :on-change #(dispatch [:change-feed-pump [[:mode :value] %]])})
       ($ f/checkbox {:value     view
                      :on-change #(dispatch [:change-feed-pump [[:mode :view] (not view)]])}))))
  ...
  (let [unit (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :performance :unit]])
    ...
    ($ f/field-row {:label "PERFORMANCE"}
       ($ f/select {:data      [{:value "%" :label "%"}]
                    :value     unit
                    :on-change #(dispatch [:change-feed-pump [[:performance :unit] %]])})
       ($ f/checkbox {:value     view
                      :on-change #(dispatch [:change-feed-pump [[:performance :view] (not view)]])}))))
  ...
  (let [unit (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :rotation :unit]])
    ...
    ($ f/field-row {:label "ROTATIONAL SPEED"}
       ($ f/select {:data      [{:value "rpm" :label "rpm"}]
                    :value     unit
                    :on-change #(dispatch [:change-feed-pump [[:rotation :unit] %]])})
       ($ f/checkbox {:value     view
                      :on-change #(dispatch [:change-feed-pump [[:rotation :view] (not view)]])}))))
  ...
  (let [unit (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :flow-rate :unit]])
    ...
    ($ f/field-row {:label "FLOW-RATE"}
       ($ f/select {:data      [{:value "t/h" :label "t/h"} {:value "T/h" :label "T/h"}]
                    :value     unit
                    :on-change #(dispatch [:change-feed-pump [[:flow-rate :unit] %]])})
       ($ f/checkbox {:value     view
                      :on-change #(dispatch [:change-feed-pump [[:flow-rate :view] (not view)]])}))))
  ...
  (let [value (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :init-sim-output :performance :value]])
    ...
    ($ f/field-row {:label "PERFORMANCE"}
       ($ f/input-number {:value     value
                          :on-change #(dispatch [:change-feed-pump [[:init-sim-output :performance :value] %]])
                          :on-blur   (fn [] (f/validate-input-number value
                                                                     (:min limits)
                                                                     (:max limits)
                                                                     validate-message
                                                                     #(dispatch [:change-feed-pump [[:init-sim-output :performance :value] %]])))
                          :&         limits})
...
(defnc rotation-init-max-value []
  (let [value (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :rotation :max-value]])
    ...
    ($ f/field-row {:label "MAXIMUM ROTATIONAL SPEED"}
       ($ f/input-number {:value     value
                          :on-change #(dispatch [:change-feed-pump [[:rotation :max-value] %]])
                          :on-blur   (fn [] (f/validate-input-number value
                                                                     (:min limits)
                                                                     (:max limits)
                                                                     validate-message
                                                                     #(dispatch [:change-feed-pump [[:rotation :max-value] %]])))
                          :&         limits})
...
(defnc flow-rate-init-max-value []
  (let [converted-value (use-sub [:feed-pump-flow-rate-max-converted-value])
    ...
    ($ f/field-row {:label "MAXIMUM FLOW-RATE"}
       ($ f/input-number {:value     converted-value
                          :on-change #(dispatch [:change-feed-pump-flow-rate-max-value %])
                          :on-blur   (fn [] (f/validate-input-number converted-value
                                                                     (:min limits)
                                                                     (:max limits)
                                                                     validate-message
                                                                     #(dispatch [:change-feed-pump-flow-rate-max-value %])))
                          :&         limits})
  ...
  (let [show-settings (use-sub [:kit-data-by-path [:boiler-plant :actuators :feed :pump :settings-view]])]
    (when show-settings
      ($ modal/settings {:title      "Feedwater Pump Settings"
                         :on-default #(dispatch [:restore-defaults-feed-pump-settings])
                         :on-done    #(dispatch [:apply-feed-pump-settings])}
