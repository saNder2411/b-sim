(ns app.connect.NRR-2-61-3C.settings
...
(defnc actuator-type-init-value []
  (let [actuator-type (use-sub [:kit-data-by-keywords [:level :controllers "NRR 2-61 3C" :actuator-type]])]
    ($ f/field-row {:label "FEEDWATER ACTUATOR"}
       ($ f/select {:data      [{:value "FREQUENCY_CONTROLLED_PUMPS" :label "Pumps"} {:value "PNEUMATIC_VALVE" :label "Pneumatic Valve"}]
                    :value     actuator-type
                    :on-change #(dispatch [:change-NRR-2-61-3C [[:actuator-type] %]])}))))
...
(defnc NRR-2-61-3C-settings []
  (let [show-settings (use-sub [:kit-data-by-keywords [:level :controllers "NRR 2-61 3C" :settings-view]])
    ...
    (when (and show-settings show-settings-form-sim-start-values)
      ($ modal/settings {:title      "NRR 2-61 3C Settings"
                         :on-default #(dispatch [:restore-defaults-NRR-2-61-3C-settings])
                         :on-done    #(dispatch [:apply-NRR-2-61-3C-settings])}
