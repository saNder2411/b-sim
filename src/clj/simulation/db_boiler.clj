...
(defn- update-pressure [db p-shrift]
  (let [next-p (+ (-> db :boiler :pressure) p-shrift)]
    (-> db
        (assoc-in [:boiler :pressure] next-p)
        (assoc-in [:boiler :temperature] (t/liquid-temperature next-p)))))

(defn- update-volume [db td-volume-shift]
  (let [{:keys [value max]} (get-in db [:boiler :volume])
        next-value (->> td-volume-shift
                        (+ value)
...
(defn boiler-sim-step! [db-atom thermodynamic]
  ;;(reset! temperature (:temperature thermodynamic))
  (swap! db-atom #(-> %
                      (update-pressure (:pressure-shift thermodynamic))
                      (update-volume (:volume-shift thermodynamic))
                      update-sludge
                      update-conductivity)))
