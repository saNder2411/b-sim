(ns simulation.db-feed-pi-controller
  (:require [simulation.helpers :as h]
            [simulation.db :as s]))

(defn feed-pi-controller-mode! [db-atom v]
  (swap! db-atom assoc-in [:actuators :feed :pi-controller :mode] v))

(defn feed-pi-controller-direction! [db-atom v]
  (swap! db-atom assoc-in [:actuators :feed :pi-controller :direction] v))

(defn feed-pi-controller-integration-action-time! [db-atom v]
  (swap! db-atom #(-> %
...
(defn feed-pi-controller-target-point! [db-atom v]
...
(defn feed-pi-controller-proportional-band! [db-atom v]
...
(defn feed-pi-controller-x-next! [db-atom v]
...
(defn feed-pi-controller-sim-step! [db-atom probe-level-%]
  (swap! db-atom #(-> %
...
(comment
  (feed-pi-controller-sim-step! s/db 55)
